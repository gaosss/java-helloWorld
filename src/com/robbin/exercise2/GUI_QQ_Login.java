...
/**
* 页签组件(JTabbedPane)选项卡窗格--使用QQ 登录界面
*/
import java.awt.*;
import javax.swing.*;
public class GUI_QQ_Login extends JFrame{
//设定组件
//主页面北部
JLabel jl1;//标签
//主页面南部
...
GUI_QQ_Login win=new GUI_QQ_Login();
}
//构造函数
public GUI_QQ_Login(){
//创建组件
//创建JFrame 北部JLabel1 组件
jl1=new JLabel(new ImageIcon("images/qqdl.jpg"));
//创建JFrame 中部JPanel2 组件
jl2=new JLabel("QQ 号码",JLabel.CENTER);
jl3=new JLabel("QQ 密码",JLabel.CENTER);
...
jl5.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));// 鼠标移动到jl5 后会变成手指图标
jtf=new JTextField();
jpf=new JPasswordField();
jb4=new JButton("清除号码");//JButton 上可以放图片。new JButton(new ImageIcon("图片路径"));
...
//加入组件
//将组件添加到JPanel2 中
jp2.add(jl2);//加入QQ 号码标签
jp2.add(jtf);//加入文本框
jp2.add(jb4);//加入清除号码按扭
jp2.add(jl3);//加入QQ 密码标签
jp2.add(jpf);//加入密码框
jp2.add(jl4);//加入忘记密码
...
jp1.add(jb3);
//将面板添加到选项卡窗格上
jtp.add("QQ 号码",jp2);//第一个参数代表选项卡名称，第二个参数代表对应的面板
jtp.add("手机号码",jp3);
jtp.add("电子邮箱",jp4);
//将JLabel1 添加到Frame 北部
this.add(jl1,BorderLayout.NORTH);
//将JPanle2 添加到Frame 中部
this.add(jtp,BorderLayout.CENTER);
//将JPanel1 添加到Frame 南部
this.add(jp1,BorderLayout.SOUTH);
//窗体设置
this.setTitle("QQ 登录界面");
